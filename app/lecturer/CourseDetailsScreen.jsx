    ...
    <SafeAreaView style={styles.container}>
      ...
      <FluentLayoutShell variant="wide">
        <View style={styles.header}>
          ...
          <View style={styles.headerTitle}><FluentText variant="title" weight="bold">{course?.code}</FluentText><FluentText variant="caption" color="neutralTextSecondary">{course?.title}</FluentText></View>
          <TouchableOpacity style={styles.iconButton} onPress={() => navigation.navigate('CourseRepManagement', { courseId })}><UserPlus color={FluentTheme.colors.accent} size={22} /></TouchableOpacity>
        </View>
        <ScrollView contentContainerStyle={styles.scrollContent}>
          <View style={styles.actionRow}>
            <FluentButton title="New Attendance" variant="primary" icon={Plus} onPress={() => navigation.navigate('TeacherBroadcastScreen', { courseId, courseCode: course.code })} style={{ flex: 1, marginRight: 8 }} />
            <FluentButton title="Export PDF" variant="outline" icon={FileDown} loading={exporting} onPress={compileCoursePDF} style={{ flex: 1 }} />
          </View>
            ...
            <TouchableOpacity key={session.id} onPress={() => navigation.navigate('SessionDetails', { sessionId: session.id, courseCode: course.code })}>
              <FluentCard style={styles.sessionCard}>
                <View style={styles.sessionInfo}>
                  <View style={{ flex: 1 }}><FluentText variant="body" weight="semibold">{session.sessionName}</FluentText><View style={styles.sessionMeta}><Clock size={12} color={FluentTheme.colors.neutralTextSecondary} /><FluentText variant="caption" color="neutralTextSecondary" style={{ marginLeft: 4 }}>{session.startedAt?.toDate().toLocaleDateString()}</FluentText></View></View>
